...
const users = new Map(); // email -> { nick, email, passwordHash, createdAt }
...
function sendVerificationEmail({ email, token }) {
  ...
  return transporter.sendMail({
    ...
    html: `
      <p>Cześć!</p>
      <p>Kliknij w link, aby potwierdzić rejestrację:</p>
...
app.post("/api/register", async (req, res) => {
  ...
  if (!confirmHuman) {
    return res.status(400).json({ message: "Potwierdź, że nie jesteś botem." });
  }
  if (users.has(email)) {
    return res.status(400).json({ message: "Konto z tym e‑mailem już istnieje." });
  }
    ...
    <html lang="pl">
      ...
      <body>
        <div class="card">
          ...
          <p>Twoje konto zostało utworzone. Możesz się zalogować.</p>
...
app.post("/api/login", async (req, res) => {
  ...
  if (!user) {
    return res.status(401).json({ message: "Nieprawidłowe dane logowania lub brak weryfikacji." });
  }
...
app.post("/api/forgot", async (req, res) => {
  ...
  if (!email) {
    return res.status(400).json({ message: "Podaj email." });
  }
...
app.listen(PORT, () => {
  console.log(`Serwer działa: ${BASE_URL}`);
});
